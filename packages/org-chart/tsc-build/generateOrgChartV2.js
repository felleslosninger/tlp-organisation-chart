export function generateOrgChart(data, containerId) {
    const { nodes, layouts } = data;
    let allowedBreakpoints = { main: 1500, laptop: 992, tablet: 768 };
    let windowWidth = window.innerWidth;
    let currentLayout = provideLayout(windowWidth, allowedBreakpoints).providedLayout;
    let isMobile = windowWidth < allowedBreakpoints.tablet;
    function provideLayout(windowWidth, breakpoints) {
    ...
    function createChildren(parentSiblingsAmount, children) {
    ...
    function createNode(node, siblingsAmount, indexInRow, isLastRow) {
        ...
        if (nodeData) {
            ...
            nodeElement.className += createNodeLineClass(indexInRow, siblingsAmount, windowWidth, allowedBreakpoints, isLastRow);
            return nodeElement;
        }
        else {
            return null;
    ...
    function createColumn(column, columnWidth, siblingsAmount, additionalWidth, indexInRow, isLastRow) {
        ...
        if (typeof column.id === "string") {
            const innerColumn = createNode(column, siblingsAmount, indexInRow, isLastRow);
            ...
            if (column.component?.children) {
                columnElement.appendChild(createChildren(siblingsAmount, column.component.children));
            }
        }
        else {
    ...
    function createRow(row, isLastRow) {
        ...
        row.row.forEach((column) => {
            count++;
            const columnWidth = calculateColumnWidth(row.row.length, count, windowWidth, allowedBreakpoints, isLastRow);
            rowElement.appendChild(createColumn(column, columnWidth.width, row.row.length, columnWidth.additionalWidth, count, isLastRow));
        });
        return rowElement;
    }
    function createRowsWrapper(layout) {
    ...
    function provideLayoutClass(windowWidth) {
    ...
    if (mainContainer) {
        ...
        window.addEventListener("resize", () => {
            ...
            currentLayout = provideLayout(windowWidth, allowedBreakpoints).providedLayout;
...
function calculateColumnWidth(siblingsAmount, indexInRow, windowWidth, breakpoints, isLastRow) {
    let width = 100;
...
function createNodeLineClass(indexInRow, siblingsAmount, windowWidth, breakpoints, isLastRow) {
    //destructuring the breakpoints object
    const { main, laptop, tablet } = breakpoints;
    let className = "";
    if (siblingsAmount === 2) {
